...
export const getPdfHtml = (p: Person) => {
  // 判定：キャストかスタッフか (pinkPdfTemplate.tsのロジックを完全再現)
  ...
  const val = (key: string) => (p[key] !== undefined && p[key] !== null && p[key] !== "") ? String(p[key]) : "　";

  // 顔写真
  let photoHtml = '';
  if (p["顔写真"]) {
    const imageUrl = p["顔写真"].startsWith('http') || p["顔写真"].startsWith('data:') 
      ? p["顔写真"] 
      : `${SERVER_URL}/get_image?image_path=${encodeURIComponent(p["顔写真"])}`;
  ...
  // 🖨️ PDF全体のスタイル定義（スマホのWebView用に見やすく調整）
  return `
    <!DOCTYPE html>
    <html>
      <head>
        ...
        <style>
          ...
          .bold-text { font-size: 16px; font-weight: bold; }
      ...
      <body>
        ...
        <div class="footer">WARP Management System</div>
      </body>
    </html>
  `;
};
